...
def custom_score(game, player):
    ...
    all_moves = len(game.get_legal_moves(player) or game.get_legal_moves(game.get_opponent(player)))

    return float(own_moves) / float(all_moves + 0.1)
...
def custom_score_3(game, player):
    ...
    return float(own_moves - opp_moves) * float(all_moves/(blank_spaces+0.1))
...
class AlphaBetaPlayer(IsolationPlayer):
    ...
    def get_move(self, game, time_left):
        ...
        **********************************************************************
        ...
        if len(game.get_legal_moves()) > 0:
            best_move = game.get_legal_moves()[0]
            # optimize best_move, in case the search fails due to timeout.
            # use the first legal move by default, better than (-1, -1)
